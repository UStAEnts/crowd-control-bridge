...
module.exports = function (prefix, addresses, effects) {
  ...
  function colorToDMX(universe, color) {
    ...
    if (color.length === 4) {
      for (const address of addresses.hasOwnProperty(universe)
        ? addresses[universe]
        : []) {
        for (let cell = 0; cell < cells; cell++) {
          result[
            address + Object.keys(channels).length * cell + channels.RED - 1
          ] = parseInt(color[0], 10);
          result[
            address + Object.keys(channels).length * cell + channels.GREEN - 1
          ] = parseInt(color[1], 10);
          result[
            address + Object.keys(channels).length * cell + channels.BLUE - 1
          ] = parseInt(color[2], 10);
          result[
            address + Object.keys(channels).length * cell + channels.WHITE - 1
          ] = parseInt(color[3], 10);
        }
      }
  ...
  function intensityToDMX(universe, intensity) {
    ...
    for (const address of addresses.hasOwnProperty(universe)
      ...
      for (let cell = 0; cell < cells; cell++) {
        result[
          address + Object.keys(channels).length * cell + channels.DIMMER - 1
