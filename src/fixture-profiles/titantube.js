    ...
    if (color.length === 3) {
      ...
      for (const address of addresses.hasOwnProperty(universe)
        ...
        for (let cell in Object.keys(channels)) {
          result[address + channels[cell].RED - 1] =
            parseInt(color[0], 10) * intensityMultiplier;
          result[address + channels[cell].GREEN - 1] =
            parseInt(color[1], 10) * intensityMultiplier;
          result[address + channels[cell].BLUE - 1] =
            parseInt(color[2], 10) * intensityMultiplier;
        }
      }
